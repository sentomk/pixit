/**
 * xEdit Extension — enhanced file editing tool.
 *
 ...
 * - `replaceAll`: replace every occurrence of oldText (opt-in)
 *
 * Safety model:
 * - Ambiguous edits (oldText matches more than once without `replaceAll`)
 *   are rejected with match line numbers instead of silently replacing the
 *   first occurrence.
 * - When exact matching fails but the text differs only in whitespace or
 *   indentation, an unambiguous fuzzy match is applied automatically.
 * - A preflight pass runs on a virtual filesystem before mutating files, and
 *   if any write fails mid-batch all already-written files are rolled back.
 *
...
const multiEditSchema = Type.Object({
	...
	replaceAll: Type.Optional(
		Type.Boolean({
			description:
				"Top-level default for edit.replaceAll. Inherited by multi items that do not set their own.",
		}),
	),
});
...
// =============================================================================
// Classic-edit matching helpers (exact + fuzzy fallback)
// =============================================================================
...
/**
 * All exact occurrences of `pattern` in `content` at or after `fromOffset`.
 */
function findExactMatches(content: string, pattern: string, fromOffset: number): TextMatch[] {
	...
	while ((idx = content.indexOf(pattern, idx)) !== -1) {
		...
		idx += pattern.length;
		if (matches.length >= 1000) break; // safety valve
	}
	return matches;
}

/**
 * Whitespace-tolerant line-based fallback when exact matching fails.
 * Tries progressively looser line comparisons (rstrip → trim → unicode-normalised)
 * and only reports the matches from the loosest level that produced hits.
 * Returns an empty array when nothing matches at any level.
 */
function findFuzzyMatches(content: string, pattern: string, fromOffset: number): TextMatch[] {
	const trimmedPattern = pattern.replace(/\n$/, "");
	if (trimmedPattern.trim().length === 0) return [];

	const lines = content.split("\n");
	const patLines = trimmedPattern.split("\n");
	if (patLines.length > lines.length) return [];

	// Character offset where each content line starts.
	...
	const levels: Array<(a: string, b: string) => boolean> = [
		(a, b) => a.trimEnd() === b.trimEnd(),
		(a, b) => a.trim() === b.trim(),
		(a, b) => normaliseLineForFuzzyMatch(a) === normaliseLineForFuzzyMatch(b),
	];

	for (const eq of levels) {
		const matches: TextMatch[] = [];
		for (let i = 0; i + patLines.length <= lines.length; i++) {
			const start = offsets[i]!;
			if (start < fromOffset) continue;
			let ok = true;
			for (let p = 0; p < patLines.length; p++) {
				if (!eq(lines[i + p]!, patLines[p]!)) {
					ok = false;
					break;
				}
			}
			if (ok) {
				const lastIdx = i + patLines.length - 1;
				matches.push({ start, end: offsets[lastIdx]! + lines[lastIdx]!.length });
				if (matches.length > 1) break; // ambiguous already; no need to keep scanning
			}
		}
...
async function applyClassicEdits(
	...
	// Snapshot original contents of every target file up front. Used both as
	// the read source during apply and for rollback if a later write fails.
	const originals = new Map<string, string>();
	for (const absPath of editOrder) {
		await workspace.checkWriteAccess(absPath);
		originals.set(absPath, await workspace.readText(absPath));
	}

	// Files successfully written so far in this pass. On any failure we roll
	// all of them back to their snapshot, making a multi-file batch atomic.
	...
	for (const absPath of editOrder) {
		...
		const originalContent = originals.get(absPath)!;
		...
		for (const { index, edit } of group) {
			...
			const matches = findExactMatches(content, edit.oldText, searchOffset);
			...
			if (matches.length === 0) {
				...
				// Fallback: whitespace-tolerant fuzzy match (unambiguous only).
				const fuzzy = findFuzzyMatches(content, edit.oldText, searchOffset);
				if (fuzzy.length === 1) {
					content =
						content.slice(0, fuzzy[0]!.start) + edit.newText + content.slice(fuzzy[0]!.end);
					searchOffset = fuzzy[0]!.start + edit.newText.length;
				...
				if (fuzzy.length > 1) {
					const locs = fuzzy.map((m) => `line ${lineNumberAt(content, m.start)}`).join(", ");
					results[index] = {
						path: edit.path,
						success: false,
						message: `Whitespace-tolerant match for oldText is ambiguous in ${edit.path}: ${locs}. Include more surrounding lines to make it unique.`,
					};
					throw new Error(formatResults(results.filter(Boolean), edits.length));
				}
			...
			// Ambiguity guard: multiple exact matches without explicit opt-in.
			if (matches.length > 1 && !edit.replaceAll) {
				const locs = matches.map((m) => `line ${lineNumberAt(content, m.start)}`).join(", ");
				results[index] = {
					path: edit.path,
					success: false,
					message: `oldText appears ${matches.length} times in ${edit.path} (${locs}). Provide more surrounding context to make it unique, or set replaceAll:true.`,
				};
				throw new Error(formatResults(results.filter(Boolean), edits.length));
			}
			...
			if (edit.replaceAll && matches.length > 1) {
				...
				for (const m of [...matches].reverse()) {
					content = content.slice(0, m.start) + edit.newText + content.slice(m.end);
				}
				// Subsequent edits rescan the whole modified file.
				searchOffset = 0;
			} else {
				const m = matches[0]!;
				content = content.slice(0, m.start) + edit.newText + content.slice(m.end);
				searchOffset = m.start + edit.newText.length;
			}
			appliedPairs.add(`${edit.oldText}\0${edit.newText}`);
		...
		// Roll back everything already written so a failed batch never leaves
		// files half-edited (e.g. when a later edit fails or the disk errors).
		if (writtenFiles.size > 0) {
			let rolledBack = 0;
			for (const p of writtenFiles) {
				try {
					await workspace.writeText(p, originals.get(p)!);
					rolledBack++;
				} catch {
					// best effort; report below
				}
			}
			if (rolledBack > 0 && err instanceof Error) {
				err.message = `Rolled back ${rolledBack} file(s) to their original contents.\n${err.message}`;
			}
		}
		throw err;
	}
...
export default function (pi: ExtensionAPI) {
	pi.registerTool({
		...
		label: "edit",
		description:
			"Edit a file by replacing exact text. The oldText must match exactly (including whitespace). Supports a `multi` parameter for batch edits across one or more files, a `patch` parameter for Codex-style patches, and `replaceAll` for replacing every occurrence. Multi-match edits are rejected with match locations unless replaceAll is set; unambiguous whitespace-tolerant fuzzy matching is used as a fallback. Failed batches roll back all writes atomically.",
		promptSnippet:
		...
		promptGuidelines: [
			...
			"When oldText appears multiple times in a file, add surrounding context to disambiguate or set replaceAll:true — ambiguous edits are rejected with match line numbers",
			"If oldText fails to match exactly but differs only in whitespace/indentation, an unambiguous fuzzy match is applied automatically",
		],
		parameters: multiEditSchema,

		async execute(toolCallId, params, signal, onUpdate, ctx) {
			const { path, oldText, newText, multi, patch, replaceAll } = params;
			...
			if (multi) {
				for (const item of multi) {
					edits.push({
						...
						replaceAll: item.replaceAll ?? replaceAll,
					});
				}
			} else if (hasTopLevel && replaceAll !== undefined) {
				edits[0]!.replaceAll = replaceAll;
			}
